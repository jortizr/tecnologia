<div class="py-6">
    <div class="flex justify-center items-center">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                {{ __("Lista de ubicaciones") }}
            </h2>
            <x-badge-title :count="$this->locations->total()" />
    </div>
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <x-data-table :data="$this->locations">
            <x-slot name="toolbar">
                ...
                <div class="w-full sm:w-48">
                    @can('dashboard.locations.create')
                        <x-wireui-button
            ...
            <x-slot name="headers">
                <tr class="text-custom-dark-bg dark:text-gray-200 uppercase text-xs tracking-wider">
                    ...
                    @can('dashboard.locations.update')
            ...
            <x-slot name="dataTBody" lazy>
                ...
                <tr class="border-b border-gray-700" wire:key="model-{{ $location->id }}">
                    <td class="px-4 py-2">{{ $location->name}}</td>
                    <td class="px-4 py-2 text-center">{{ $location->creator->name ?? 'sin movimiento' }}</td>
                    <td class="px-4 py-2 text-center">{{ $location->updater->name ?? 'sin actualizacion' }}</td>
                    @can('dashboard.locations.update')
                    <td class="px-4 py-2 text-center align-middle">
                        <div class="flex justify-center items-center gap-2">
                        <x-wireui-button xs circle secondary icon="pencil" wire:click="edit({{ $location->id }})" />
                        <x-wireui-button
                                    xs
                                    circle
                                    negative
